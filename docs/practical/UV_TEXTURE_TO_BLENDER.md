...
UV 좌표는 카메라 픽셀 좌표와 **완전히 별개**입니다. 3D 메쉬 표면을 2D로 "펼친" 좌표계입니다.

- **카메라 픽셀 (u, v)**: 카메라가 찍은 사진에서 피사체의 위치
- **UV 좌표 (u, v)**: 동물 가죽을 벗겨서 바닥에 펼쳤을 때 각 부위의 위치

### 1.2 데이터 구조

```
mouse_model/mouse_txt/
├── vertices.txt      # (7894, 3) 정점의 3D 위치 (템플릿, mm)
├── faces_vert.txt    # (15784, 3) 삼각형 면 → 정점 인덱스 3개
├── textures.txt      # (8043, 2) UV 좌표 (0~1 범위)
└── faces_tex.txt     # (15784, 3) 삼각형 면 → UV 인덱스 3개
```

UV 좌표 수(8043) > 정점 수(7894)인 이유: 이음새(seam)에서 하나의 3D 정점이 UV 공간에서 2개로 분리됨 (지구본→세계지도 전개와 동일).

### 1.3 매핑 관계

```
faces_vert[0] = [100, 203, 457]   # 정점 #100, #203, #457이 삼각형
faces_tex[0]  = [112, 220, 480]   # 이 삼각형의 UV 좌표 인덱스

vertices[100]  = (85.3, 22.1, 30.5)   # 3D 위치 (코 근처)
textures[112]  = (0.73, 0.45)         # UV 공간 위치

3D 공간 (메쉬)                 UV 공간 (텍스처 이미지 512x512)

   v457 ·                      px=(0.72*512, 0.44*512) ·
       / \                                             / \
      /   \  ── UV 매핑 ──→                          /   \
     /     \                                        /     \
v100 ·─────· v203              (374, 230) ·────────· (378, 235)
```

### 1.4 텍스처 생성 과정 (UV Pipeline)

```
Step 1: 카메라 투영 (MVG 사용)
   3D 정점 (85.3, 22.1, 30.5)
        │  K @ [R|T]  (카메라 캘리브레이션)
        ▼
   카메라 픽셀 (312, 198)
        │  원본 영상에서 색상 샘플링
...
Step 2: UV 공간에 색상 기록
   이 정점의 UV 좌표 = (0.73, 0.45)
        │  px = 0.73 * 512 = 374
        ▼
   texture_final.png[230, 374] = RGB(142, 98, 73)
```

6개 카메라에서 반복 → visibility 가중 평균 → 텍스처 완성.

### 1.5 왜 텍스처 1장으로 모든 프레임에 재사용 가능?

| 개념 | 역할 | 프레임마다 변하는가? |
|------|------|---------------------|
| **OBJ (vertices)** | 정점의 3D 위치 | **변함** (포즈) |
| **UV 좌표** (textures.txt) | 정점 → 텍스처 위치 매핑 | 고정 (body model 정의) |
| **텍스처** (texture_final.png) | UV 공간의 색상 이미지 | 고정 (한 번 생성) |
| **카메라 K,R,T** | 3D→2D 투영 (생성 시에만 사용) | 카메라별 고정 |

포즈가 바뀌면 정점의 3D 위치만 변하고, UV 좌표(어떤 정점이 텍스처의 어디를 참조하는지)는 body model에 고정이므로 **텍스처 1장 = 모든 프레임 재사용**.
...
### 4.3 추가 처리 (export_to_blender.py 기본값)
